...
static int *LRUStack;//array simulates doubly linked list
...
StrategyUpdateAccessedBuffer(int buf_id, bool delete)
{
	...
	int i;//iterator
	int j;//iterator or temp storage for buf_id
	bool has_bid = false;
	
	if(delete){
		// search buf_id for a match
		printf("delete");
		for(i = 0; i <= end; i++){
			...
			if(LRUStack[i] == buf_id){
				...
				printf("delete buffer: %i\n",LRUStack[i]);
				return;
			}
		}
		printf("no delete buffer found\n");
		// not found
	}else{
		for(i = 0; i <= end; i++){
			if(LRUStack[i] == buf_id){
		...
		if(has_bid){
			...
			LRUStack[0] = j;
			printf("update buffer %i to first\n",j);
		// insert case
		}else{
			StrategyControl->stackEnd = ++end;
			for(i = end; i > 0; i--){
